...
To set up your webhook, open the webhooks section in your bot/server's Top.gg page settings, and enter your full recipient URL in the `Webhook URL` box. In your example, it's `https://yourserver.com/topgg-webhook`.
...
app.post('/topgg-webhook', (req, res) => {
  if (req.headers['Authorization'] === TOPGG_WEBHOOK_SECRET) {
    // highlight-start
    const voterUserId = req.body.user;
    ...
    fetch(DISCORD_WEBHOOK_URL, {
      ...
      body: JSON.stringify({
        username: 'Top.gg',
        content: `A user with the ID of \`${voterUserId}\` has voted us on Top.gg!`
      })
    });
    // highlight-end
